...
| **Can we put our own data in?** | Not today. That is the feature we are building: drop the scene on the site, it processes on the machine running the pipeline, and the finished case appears in the picker in a few minutes. See [document 10](10-UI-WALKTHROUGH.md) §10.12 for the exact wording. |
| **How big is all this?** | The raw datasets stay outside the shipped product entirely — terabytes of `Oil/` and `Mask_oil/` are never copied in. The build asserts on every run that the bundled files contain no dataset formats, no host paths and no credentials. What ships is the interface: **399 KB of code** (JavaScript, CSS, HTML) plus the demo case's images and JSON, **2.9 MB in total** — that is the whole working product, offline. |
