...
| **How big is all this?** | The raw datasets stay outside the shipped product entirely — terabytes of `Oil/` and `Mask_oil/` are never copied in. The build asserts on every run that the bundled files contain no dataset formats, no host paths and no credentials. What ships is the interface: **477 KB of code** (JavaScript, CSS, HTML) plus the demo case's images and JSON, **3.0 MB in total** — that is the whole working product, offline. |
